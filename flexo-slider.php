...
public static	function nggGetSliderPics($chosen_gallery) {
	...
	//	echo "<pre>";print_r ($chosen_gallery);echo "</pre>";
		...
		$dirresult = $wpdb->get_results($query);
			$sliderPicsHTML = '';
			if(!$chosen_gallery ['max_br']){$max_br=count($results);}
			else{$max_br=$chosen_gallery ['max_br'];}
				for ($i = 0; $i < $max_br; $i++ ) {
			...
			/*  proverka true/false za title  i link*/
				...
				if($chosen_gallery['img_title']=='true'):
								...
								if($chosen_gallery ['img_link']=='true') {
									...
									foreach ($new_rezult as $n_rez) {
										if($chosen_gallery['img_des']=='true'){$img_des='&#13;&#10;'.$n_rez->description;}
										else{$img_des=' ';}
									$sliderPicsHTML .= '<a href="'.$n_rez->description.'"><img src="'.$dirresult[0]->path .'/'.$n_rez->filename.'" alt="'.$n_rez->alttext.'" title="'.$n_rez->alttext.$img_des.'"></a>';
									}	
								}
								else {
									...
									$sliderPicsHTML .= '<img src="'.$dirresult[0]->path.'/'.$n_rez->filename . '" alt="'.$n_rez->alttext.'"  title="'.$n_rez->alttext.$img_des.'">';
									}
								}
								...
								if($chosen_gallery ['img_link']=='true') {
									...
									foreach ($new_rezult as $new_rezult) {
										if($chosen_gallery['img_des']=='true'){$img_des='</br>'.$new_rezult->description;}
										else{$img_des='';}
										if($chosen_gallery['img_des']=='true'){$img_des='</br>'.$new_rezult->description;}
										else{$img_des='';}
									$sliderPicsHTML .= '<a href="' . $new_rezult->description . '"><img src="' . $dirresult[0]->path . '/' . $new_rezult->filename . '" alt="' . $new_rezult->alttext. '" title="'.$img_des.'"></a>';
									}	
								}
								else {
									foreach ($new_rezult as $new_rezult) {
									...
									$sliderPicsHTML .= '<img src="' . $dirresult[0]->path . '/' . $new_rezult->filename . '" alt="' . $new_rezult->alttext.'" title="'.$img_des.'">';
									}
								}
			
				endif;
			//	$sliderPicsHTML .=$new_rezult->description;
			//echo "<pre>";print_r ($sliderPicsHTML);echo "</pre>";
...
public static function get_gallery ($_params) {
	...
	//echo "<pre>";print_r ($pic_array);echo "</pre>";
	...
	while(($spos = strpos($content,$pattern,++$epos)) > -1):
		...
		if($epos != -1):
			...
			if(preg_match_all('/(?<key>[^=]+)=[\'"](?<val>[^\'"]+)[\'"]/i',$settings,$m)):
				...
				endforeach;
				$test=$params['GalleryID'];
				$gall=explode(',',$test);
				//$params['GalleryID']=$gall;
			//echo "<pre>";print_r ($params); echo "</pre>";
			
			foreach($gall as $gID){
				$params['GalleryID']=$gID;
				$flexoslider_html_gallery .= flexoslider::nggGetSliderPics($params);
				
			}
		
	//	echo "<pre>";print_r ($flexoslider_html_gallery); echo "</pre>";
...
?>
	<div id="flexo_slider_page">
			<form method="POST">
				...
				</br>Gallery ID <input style="width: 100px;" type="text" name="GalleryID" value="<?php echo $GalleryID; ?>" />
				(for more than one, separated by commas)
				</br>Effect:
				<table>
								<tr>
				...
				</br>Speed of animation in milliseconds <input style="width: 100px;" type="text" name="animSpeed" value="<?php echo $animSpeed; ?>"/>
				
				</br>Time for the Pause in milliseconds <input style="width: 100px;" type="text" name="pauseTime" value="<?php echo $pauseTime; ?>"/>
				
				</br>Width <input style="width: 100px;" type="text" name="width" value="<?php echo $width; ?>"/>
				
				</br>Height <input style="width: 100px;" type="text" name="height" value="<?php echo $height; ?>"/>
				
				</br>Link on Picture <input  type="radio" name="img_link" value="true" checked <?php echo $img_link == 'true' ? 'checked' : ''; ?> /> yes
									 						<input  type="radio" name="img_link" value="false"<?php echo $img_link == 'false' ? 'checked' : ''; ?>/> no
									 						
				</br>Display Picture Name	<input  type="radio" name="img_title" value="true" checked <?php echo $img_title == 'true' ? 'checked' : ''; ?> /> yes
									 									<input  type="radio" name="img_title" value="false" <?php echo $img_title == 'false' ? 'checked' : ''; ?>/> no
				</br>Display Picture Description	<input  type="radio" name="img_des" value="true" checked <?php echo $img_des == 'true' ? 'checked' : ''; ?> /> yes
									 									<input  type="radio" name="img_des" value="false" <?php echo $img_des == 'false' ? 'checked' : ''; ?>/> no
									 									
				</br>Display navigation	<input  type="radio" name="img_nav" value="true" checked <?php echo $img_nav == 'true' ? 'checked' : ''; ?> /> yes
									 						<input  type="radio" name="img_nav" value="false" <?php echo $img_nav == 'false' ? 'checked' : ''; ?>/> no
									 						
				</br>Count Limit <input  type="text" name="max_br" value="<?php echo $max_br; ?>"  /> 
				
				</br>Order algorithm <input  type="radio" name="order" value="order" checked <?php echo $order == 'order' ? 'checked' : ''; ?> /> order
									 					 <input  type="radio" name="order" value="random" <?php echo $order == 'random' ? 'checked' : ''; ?>/> random
									 						
				</br>Pause when Mouse is on the Pictures <input  type="radio" name="pause" value="true" checked <?php echo $pause == 'true' ? 'checked' : ''; ?> /> yes
									 									 		 <input  type="radio" name="pause" value="false" <?php echo $pause == 'false' ? 'checked' : ''; ?>/> no
									 					
				</br>Navigation arrows <input  type="radio" name="nav_arrow" value="true" checked <?php echo $nav_arrow == 'true' ? 'checked' : ''; ?> /> yes
...
<?php		
	...
	public static function flexo_slider_menu () {
		add_options_page('admin slider','Flexo Slider','manage_options','slider-manager','flexoslider::flexo_slider_option','');
	}
...
if(function_exists('add_action')):
	if (!is_admin()  ) {
		...
		add_filter('the_content','flexoslider::filter',1);
	}else{
